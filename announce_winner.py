...
class ОбъявлениеПобедителя:
    ...
    def загрузить_метаданные(self) -> dict | None:
        ...
        try:
            ...
            запрос1_рус = данные.get('img1_query_ru', перевести(данные.get('img1_query', '')))
            запрос2_рус = данные.get('img2_query_ru', перевести(данные.get('img2_query', '')))
            logger.info(f"🆚 {запрос1_рус} vs {запрос2_рус}")
            return данные
    ...
    def отправить_объявление(self, результат: dict, метаданные: dict) -> bool:
        ...
        # Получаем русские названия из метаданных или переводим
        запрос1_рус = метаданные.get('img1_query_ru', перевести(метаданные['img1_query']))
        запрос2_рус = метаданные.get('img2_query_ru', перевести(метаданные['img2_query']))
        ...
        if результат['победитель'] == 'ничья':
            текст = (
                ...
                f"📊 **Результаты:**\n"
                f"🔥 {запрос1_рус}: {результат['процент_первого']}% ({результат['голоса_первого']} голосов)\n"
                f"❤️ {запрос2_рус}: {результат['процент_второго']}% ({результат['голоса_второго']} голосов)\n"
            ...
            победитель_рус = результат['победитель_запрос']
            проигравший_рус = результат['проигравший_запрос']
            
            текст = (
                f"🏆 **ИТОГИ БИТВЫ** 🏆\n\n"
                f"🎉 **{победитель_рус}** победил **{проигравший_рус}**!\n\n"
                f"📊 **Результаты:**\n"
                f"🔥 {запрос1_рус}: {результат['процент_первого']}% ({результат['голоса_первого']} голосов)\n"
                f"❤️ {запрос2_рус}: {результат['процент_второго']}% ({результат['голоса_второго']} голосов)\n"
                f"🤝 Оба классные: {результат['процент_обоих']}% ({результат['голоса_обоих']} голосов)\n\n"
        ...
        if результат['победитель'] == 'первый':
            ...
            return self.отправить_ничью(текст, метаданные, запрос1_рус, запрос2_рус)
    ...
    def отправить_ничью(self, текст: str, метаданные: dict, запрос1_рус: str, запрос2_рус: str) -> bool:
